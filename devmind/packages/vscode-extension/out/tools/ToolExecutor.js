...
class ToolExecutor {
    ...
    normalizeArgs(name, args) {
        ...
        if (name === 'view_file' && Array.isArray(normalized.view_range) && normalized.view_range.length >= 2) {
            if (!normalized.startLine)
                normalized.startLine = normalized.view_range[0];
            if (!normalized.endLine)
                normalized.endLine = normalized.view_range[1];
        }
        // replace_file_content: handle alternate arg names
        if (name === 'replace_file_content') {
            if (!normalized.targetText)
                normalized.targetText = normalized.target_text || normalized.old_str || normalized.old_string || normalized.old_text || normalized.search || normalized.find;
            if (!normalized.replacementText)
                normalized.replacementText = normalized.replacement_text || normalized.new_str || normalized.new_string || normalized.new_text || normalized.replace || normalized.new_content;
        }
        // grep_search / codebase_search: handle alternate search arg names
        if (!normalized.query)
            normalized.query = normalized.search_term || normalized.regex || normalized.text;
    ...
    async executeTool(name, args) {
        try {
            ...
            switch (name) {
                ...
                case 'view_code_item': {
                    if (!args.itemName)
                        return `Tool Execution Error: Missing required argument "itemName".`;
                    return await this.fileReader.viewCodeItem(args.path, args.itemName);
                }
                // File Writing
                case 'write_to_file': {
                    if (args.content === undefined)
                        return `Tool Execution Error: Missing required argument "content".`;
                    return await this.fileWriter.writeToFile(args.path, args.content, args.overwrite);
                }
                case 'replace_file_content': {
                    if (args.targetText === undefined || args.replacementText === undefined)
                        return `Tool Execution Error: Missing required arguments "targetText" or "replacementText".`;
                ...
                case 'multi_replace_file_content': {
                    if (!args.replacements || !Array.isArray(args.replacements))
                        return `Tool Execution Error: Missing or invalid "replacements" array.`;
                ...
                case 'grep_search': {
                    ...
                    if (!args.query)
                        return `Tool Execution Error: Missing required argument "query".`;
                    if (!args.path)
                        return `Tool Execution Error: Missing required argument "path" (or SearchPath).`;
                ...
                case 'find_by_name': {
                    if (!args.pattern)
                        return `Tool Execution Error: Missing required argument "pattern".`;
                ...
                case 'run_command': {
                    if (!args.command && !args.CommandLine)
                        return `Tool Execution Error: Missing required argument "command".`;
                ...
                case 'command_status': {
                    if (!args.commandId)
                        return `Tool Execution Error: Missing required argument "commandId".`;
                    return await this.terminal.commandStatus(args.commandId);
                }
                case 'send_command_input': {
                    if (!args.commandId || args.input === undefined)
